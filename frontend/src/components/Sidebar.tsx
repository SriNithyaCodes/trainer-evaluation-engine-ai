...
export default function Sidebar({ onLogout }: { onLogout: () => void }) {
  return (
    <aside className="w-72 flex flex-col p-8 z-20 bg-slate-50 border-r border-slate-100 min-h-screen">
      {/* Logo */}
      ...
      <nav className="flex-1 space-y-1.5">
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-4 px-4">
          Main Navigation
        </p>
        {links.map((link) => (
          <NavLink
      ...
      <div className="pt-8 border-t border-slate-200 space-y-1.5">
        <button className="w-full flex items-center gap-3 px-4 py-3.5 text-slate-500 hover:text-slate-900 hover:bg-white/60 rounded-xl transition-all text-sm font-semibold">
